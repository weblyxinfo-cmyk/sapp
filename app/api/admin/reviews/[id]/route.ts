...
export async function PUT(request: Request, ctx: Ctx) {
  try {
    ...
    await db.execute({
      sql: 'UPDATE reviews SET text = ?, author = ?, location = ?, date = ?, visible = ?, sort_order = ? WHERE id = ?',
      args: [text, author, location || null, date || null, visible ? 1 : 0, sort_order || 0, Number(id)],
    });
...
export async function PATCH(_request: Request, ctx: Ctx) {
  try {
    ...
    await db.execute({
      sql: 'UPDATE reviews SET visible = CASE WHEN visible = 1 THEN 0 ELSE 1 END WHERE id = ?',
      args: [Number(id)],
    });
